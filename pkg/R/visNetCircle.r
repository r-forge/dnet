...
#' visNetCircle(g=g, com=com, ordering=ordering, vertex.label=V(g)$name)
#'
#' # 6b) drawn into multlpe circles (one circle per community) 
#' visNetCircle(g=g, com=com, circles="multiple", ordering=ordering, vertex.label=V(g)$name)
#'
...
#' visNet(g, glayout=layout.fruchterman.reingold, vertex.color=vcolors, vertex.frame.color=vcolors, vertex.shape="sphere", mark.groups=mark.groups, mark.col=mark.col, mark.border=mark.border, mark.shape=1, mark.expand=10, edge.color=edge.color)
