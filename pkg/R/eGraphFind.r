...
#' }
#' @export
#' @seealso \code{\link{eGraphFind}}
#' @include eGraphFind.r
#' @examples
...
eGraphFind <- function(g, scores)
{
    ...
    if(length(pos.nodes)==0){
        ...
        module <- eGraphInduce(ig, pos.nodes, knn=0, remove.loops=T, largest.comp=T)
    }else{
        
        pos.subgraph <- eGraphInduce(ig, pos.nodes, knn=0, remove.loops=T, largest.comp=F)
        ...
        if(length(mst.cluster.id)==1){
            ...
            sub.mig <- eGraphInduce(mig, V(mst)[all.ids]$name, knn=0, remove.loops=T, largest.comp=F)
        ...
        if (length(neg.node.ids.2) == 0) {
            ...
            module <- eGraphInduce(ig, tmp_nodes, knn=0, remove.loops=F, largest.comp=F)
            #module <- eGraphInduce(ig, tmp_nodes, knn=0, remove.loops=F, largest.comp=T)
            
        }else{
            ## connected graph between borders (neg.node.ids.2)
            subg <- eGraphInduce(sub.mig, V(sub.mig)[neg.node.ids.2]$name, knn=0, remove.loops=T, largest.comp=T)
