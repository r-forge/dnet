...
#'
#' \code{visGraphReorder} is supposed to visualise the multiple graph colorings reorded within a sheet-shape rectangle grid
#'
...
#' @note none
#' @export
#' @seealso \code{\link{visGraph}}, \code{\link{eGraphReorder}}
#' @include visGraphReorder.r
#' @examples
...
#' sReorder <- eGraphReorder(g=subg, data, feature="node", node.normalise="none")
#' 
#' # 4) visualise the module with vertices being color-coded by input data
#' visGraphReorder(g=subg, data=data, sReorder)

visGraphReorder <- function (g, data, sReorder, height=7, margin=rep(0.1,4), border.color="#EEEEEE", colormap=c("bwr","jet","gbr","wyr","br","yr","rainbow","wb"), ncolors=40, zlim=NULL, colorbar=T, colorbar.fraction=0.5, newpage=T, glayout=layout.fruchterman.reingold, mtext.side=3, mtext.adj=0,mtext.cex=1,mtext.font=2,mtext.col="black", ...)
{
    ...
    for(k in 1:length(cnames)){
        visGraph(ig, glayout=glayout_fix, pattern=data[,k], colormap=colormap, ncolors=ncolors, zlim=zlim, colorbar=F, newpage=F, ...)
