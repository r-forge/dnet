...
#'
#' \code{eWrite} is supposed to write out enrichment results. 
#'
...
#' @export
#' @seealso \code{\link{eCal}}
#' @include eWrite.r
#' @examples
#' #output <- eWrite(eTerm, which_content="gadjp", which_score="gadjp", cutoff=0.05, filename="eWrite_output.txt", keep.significance=T)

eWrite <- function(eTerm, which_content=c("gadjp","adjp","pvalue","FWER","FDR","qvalue","nES","ES"), which_score=c("gadjp","adjp","FWER","FDR","qvalue"), cutoff=0.1, filename=NULL, keep.significance=T) 
{
