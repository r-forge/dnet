...
#' \item{Where \eqn{\tau =\big[\frac{\lambda+(1-\lambda)*a-fdr*\lambda}{fdr*(1-\lambda)}\big]^{\frac{1}{a-1}}}, i.e. the p-value corresponding to the exact \eqn{fdr} threshold. It can be duduced from the definition of the false discovery rate: \eqn{fdr \doteq \frac{\int_0^\tau \lambda+(1-\lambda)*a \, \mathrm{d}x}{\int_0^\tau \lambda+(1-\lambda)*a*x^{a-1} \, \mathrm{d}x}}. Notably, if the calculated \eqn{\tau} exceeds the contraint, it will be reset to the maximum end of that constraint}
#' }
#' @export
#' @seealso \code{\link{eBUMfit}}
#' @include eBUMscore.r
#' @examples
...
#' # using "pdf" method
#' scores <- eBUMscore(fit, method="pdf", fdr=0.01)
#' # using "cdf" method
#' scores <- eBUMscore(fit, method="cdf", fdr=0.01)

eBUMscore <- function(fit, method=c("pdf","cdf"), fdr=NULL, scatter.bum=T)
{
