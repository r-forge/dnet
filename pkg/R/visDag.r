...
#' }
#' @export
#' @importFrom graph sparseM2Graph edgeData
...
#'
#' # 4) given term distance to color-code nodes
#' data <- V(subg)$term_distance
#' names(data) <- V(subg)$term_id
#' visDag(g=subg, data=data, node.info="both")
#' # 4a) globally change the node and edge attributes
...
#' # 4b) locally highlight the root by changing its shape into "box"
#' root.shape <- "box"
#' names(root.shape) <- V(subg)[which(V(subg)$term_distance==0)]$name
#' visDag(g=subg, data=data, node.info="both", node.attrs=list(shape=root.shape))
#' # 4c) further locally remove the root labelling
#' root.label <- ""
#' names(root.label) <- V(subg)[which(V(subg)$term_distance==0)]$name
...
visDag <- function (g, data=NULL, height=7, width=7, margin=rep(0.1,4), colormap=c("lightyellow-orange","yr","bwr","jet","gbr","wyr","br","rainbow","wb"), ncolors=40, zlim=NULL, colorbar=T, colorbar.fraction=0.1, newpage=T, layout.orientation=c("left_right","top_bottom","bottom_top","right_left"), node.info=c("none", "term_id", "term_name", "both", "full_term_name"), graph.node.attrs=NULL, graph.edge.attrs=NULL, node.attrs=NULL)
{
    ...
    if(layout.orientation=="bottom_top" || layout.orientation=="right_left"){
        ig <- igraph.reverse(ig)
    }
    
    dag <- igraph.to.graphNEL(ig)
    if(layout.orientation=="bottom_top" || layout.orientation=="right_left"){
        #dag <- graph::reverseEdgeDirections(dag)
    }
