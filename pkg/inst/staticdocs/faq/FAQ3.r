...
# For grouped samples, the the p-value significance for genes can be calculated using the package 'limma' which is widely used for idnetification of differentially expressed genes. The example for this can be found in the <a href="http://dnet.r-forge.r-project.org/demo-Hiratani.html">Demo for multilayer replication timing dataset</a> page.

# For time-course samples, the dnet package has developed a singular value decomposition-based method to obtain gene significance (see dSVDsignif). The example for this can be found in the <a href="http://dnet.r-forge.r-project.org/demo-Fang.html">Demo for time-course expression dataset</a> page.

# For survival samples, survival analysis should be applied using Cox proportional hazards model, from which Cox hazard ratio and associated p-value for each gene can be calculated. This can be done via the 'survival' package, and is demonstrated in the <a href="http://dnet.r-forge.r-project.org/demo-TCGA.html">Demo for TCGA mutation and survival dataset</a> page.
